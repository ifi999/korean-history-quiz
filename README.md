...
| 4시간 근현대사 수업자료 | 수업자료 기반 빈칸 회상 | 개항기, 일제강점기, 현대사 | 33문항 | 승인 완료 |
| 5시간 전근대사 수업자료 | 객관식/순서 배열 | 고대, 고려, 조선 | 36문항 | 승인 완료 |
| 5시간 전근대사 수업자료 | 수업자료 기반 빈칸 회상 | 고대, 고려, 조선 | 23문항 | 승인 완료 |
...
- 채점형 승인 문항: 217문항
- 승인 복습 카드: 85카드
- draft 자료: 백지 테스트 카드, 일부 초안 문항, 판독 확인이 필요한 손글씨 concept_note
...
| 말하는 방식 | 동작 |
| --- | --- |
| `퀴즈 시작`, `문제 내줘`, `한국사 해보자`, `랜덤 문제`, `골고루 내줘` | 기본 승인 문항에서 balanced random으로 한 문제씩 출제 |
| `전근대`, `근현대`, `고려`, `조선`, `현대사`, `파이널1` | 범위 필터링 |
| `객관식`, `O/X`, `순서`, `빈칸`, `단답`, `수업자료`, `키워드 카드` | 문제/카드 유형 필터링 |
| `정해진 문항 말고`, `내용 기반`, `주관식`, `백지 테스트`, `꼬리질문` | 수업자료 concept_note에서 즉석 주관식 출제 |
...
- 기본 출제는 파일 순서가 아니라 balanced random입니다. 전체 범위에서는 자료/챕터/유형이 한쪽으로 몰리지 않게 섞습니다.
...
- 수업자료에서 확실히 추출된 concept_note는 웹 검증 없이 빈칸/단답 회상형 문항으로 변환할 수 있습니다. 이 경우 문항은 PDF 출처와 source_extracted 근거 Fact를 함께 가집니다.
- 사용자가 `정해진 문항 말고 내용 기반으로`라고 요청하면 Codex는 고정 문항만 반복하지 않고 `source_extracted` concept_note의 `title`, `summary`, `keyPoints`에서 즉석 주관식 질문을 만듭니다.
