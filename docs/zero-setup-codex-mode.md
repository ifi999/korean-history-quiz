...
3. 후보를 balanced random으로 섞어 한 문제를 낸다.
...
기본 출제는 순차 출제가 아닙니다. 전체 범위에서는 source set, chapter, tag, questionType이 한쪽으로 몰리지 않게 섞고, 특정 범위 안에서도 pageNumber나 itemNumber가 연속되지 않게 고릅니다. 사용자가 "처음부터 순서대로"라고 명시한 경우에만 순차 출제를 허용합니다.
